...
@Observable
@MainActor
final class CKSocialInteractionService: SocialInteractionService {
    private(set) var likeCounts: [String: Int] = [:]
    private(set) var commentCounts: [String: Int] = [:]
    private(set) var likedEncounters: Set<String> = []
    ...
    init(getCurrentUserID: @escaping @Sendable () -> String?) {
    ...
    #if DEBUG
    func seedFakeInteractions(encounterRecordNames: [String]) {
