# App Store Metadata

## App Name

Catch

## Subtitle (30 chars max)

Log every cat you meet! 

## Short Description

Track and collect the cats you encounter in the wild & share with friends

## Long Description

catch is a cat tracker

spot a cat? log it. snap a photo, drop a pin, let the app figure out the breed. every encounter gets saved: where you were, what the cat looked like, whether it gave you the time of day or simply walked away mid-pet.

features:
- log cat encounters with photos, location, and notes
- automatic breed identification powered by on-device AI
- map view showing every cat you've ever crossed paths with
- breed collection log
- social feed — follow friends, like their sightings, leave comments
- full encounter history for every cat you track

no ads. no tracking. no subscriptions. just cats.

## Keywords (100 chars max, comma separated)

cat tracker,cat log,stray cats,pet tracker,cat diary,cat map,breed identifier,cat collection,meow

## Category

- **Primary**: Lifestyle
- **Secondary**: Social Networking

## Age Rating

- **Rating**: 4+ (no objectionable content)
- Unrestricted Web Access: No
- Gambling / Contests: No
- Horror / Fear Themes: No
- Medical / Treatment Info: No
- Profanity / Crude Humor: No
- Mature / Suggestive Themes: No
- Simulated Gambling: No
- Alcohol / Tobacco / Drug Use or References: No
- Sexual Content / Nudity: No
- Graphic Violence (Cartoon / Fantasy / Realistic / Prolonged): No

**Note**: The app uses location data (When In Use) but this does not affect the age rating.

## Copyright

Copyright 2026 Grayson Martin

## Version

1.0

## App Icon

1024x1024 PNG. Orange paw print on a dark green gradient background. Located at `catch/Assets.xcassets/AppIcon.appiconset/AppIcon.png`. Uses Xcode's modern single-size icon format (universal, iOS platform). Renders correctly at all required sizes via automatic downscaling.
